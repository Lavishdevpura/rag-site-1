...
# Domain confirmed 2026-08-19 (research doc shared via Teams/ChatGPT export):
# reinsurance quote-assistance support for agents/underwriters — treaty vs
# facultative, proportional vs non-proportional (XOL/Cat XL/quota share/
# surplus), IRDAI reinsurance regulation, and market/underwriting practice
# (Lloyd's, Munich Re). Persona name is a single swap-in constant so it
# doesn't need hunting through every prompt below.
PERSONA_NAME = "Reinsurance Assistant"
...
17. CONTEXT chunks are ordered by relevance - build the answer from the first (best-matching) chunk when it directly answers the question; don't let a less-relevant later chunk (sharing only a keyword) replace it.
...
24. KNOWLEDGE BASE chunks are ordered by relevance - build the answer from the first (best-matching) chunk when it directly answers the question; don't let a less-relevant later chunk (sharing only a keyword) replace it.
...
18. KNOWLEDGE BASE chunks are ordered by relevance - at least one point must be built from the first (best-matching) chunk when it directly answers the question; don't let less-relevant later chunks (sharing only a keyword) crowd it out entirely.
